...
const FLAG_ADDRESS = "0x20a1A5857fDff817aa1BD8097027a841D4969AA5"
const flag = new Contract(FLAG_ADDRESS, FLAG_ABI, executorWallet)

export async function claimFlag3() {
  ...
  mevshare.on(StreamEventType.Transaction, async (pendingTx: IPendingTransaction) => {
    ...
    if (!pendingTx || !!pendingTx.logs || !!pendingTx.to || !!pendingTx.functionSelector) { return }

    if (!pendingTx.logs && !pendingTx.to) {
